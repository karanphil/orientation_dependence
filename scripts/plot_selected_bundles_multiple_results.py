...
def _build_arg_parser():
    ...
    p.add_argument('--results_1', nargs='+', default=[],
                   action='append', required=True,
                   help='List of characterization results.')
    p.add_argument('--results_2', nargs='+', default=[],
                   action='append',
                   help='List of characterization results.')
    p.add_argument('--results_3', nargs='+', default=[],
                   action='append',
                   help='List of characterization results.')
    p.add_argument('--results_4', nargs='+', default=[],
                   action='append',
                   help='List of characterization results.')
    p.add_argument('--results_5', nargs='+', default=[],
                   action='append',
                   help='List of characterization results.')
    ...
    p.add_argument('--whole_WM_1', default=[],
                   help='Path to the whole WM characterization.')
    p.add_argument('--whole_WM_2', default=[],
                   help='Path to the whole WM characterization.')
    p.add_argument('--whole_WM_3', default=[],
                   help='Path to the whole WM characterization.')
    p.add_argument('--whole_WM_4', default=[],
                   help='Path to the whole WM characterization.')
    p.add_argument('--whole_WM_5', default=[],
                   help='Path to the whole WM characterization.')
...
def main():
    ...
    all_results_args = [args.results_1, args.results_2, args.results_3,
                        args.results_4, args.results_5]
    
    all_whole_wm_args = [args.whole_WM_1, args.whole_WM_2, args.whole_WM_3,
                         args.whole_WM_4, args.whole_WM_5]

    all_results = []
    all_extracted_bundles = []
    max_count = 0
    for results_args in all_results_args:
        results = []
        extracted_bundles = []
        tmp = 0
        if results_args:
            for i, result in enumerate(results_args[0]):
                if str(Path(result).parent.name) in bundles_names:
            ...
            all_extracted_bundles.append(extracted_bundles)

    all_whole_wm = []
    all_whole_mid_bins = []
    for whole_wm_args in all_whole_wm_args:
        if whole_wm_args:
            print("Loading: ", whole_wm_args)
            whole_wm = np.load(whole_wm_args)
            whole_mid_bins = (whole_wm['Angle_min'] + whole_wm['Angle_max']) / 2.
            all_whole_wm.append(whole_wm)
            all_whole_mid_bins.append(whole_mid_bins)
    ...
    nb_rows = nb_bundles
    min_mtr = np.ones((nb_bundles + 1)) * 10000
    max_mtr = np.zeros((nb_bundles + 1))
    min_mtsat = np.ones((nb_bundles + 1)) * 10000
    max_mtsat = np.zeros((nb_bundles + 1))
    min_ihmtr = np.ones((nb_bundles + 1)) * 10000
    max_ihmtr = np.zeros((nb_bundles + 1))
    min_ihmtsat = np.ones((nb_bundles + 1)) * 10000
    max_ihmtsat = np.zeros((nb_bundles + 1))
    ...
    color_idx = 1
    for results, whole_wm, whole_mid_bins, extracted_bundles in zip(all_results, all_whole_wm, all_whole_mid_bins, all_extracted_bundles):
        color_idx += 1
        ...
        for i in range(nb_bundles):
            ...
            if bundles_names[i] in extracted_bundles:
                ...
                colorbar = ax[row, col].scatter(mid_bins[is_measures],
                                                ...
                                                edgecolors=cm.naviaS(color_idx), linewidths=1)
                ...
                min_mtr[bundle_idx] = np.min((np.nanmin(result['MTR']), min_mtr[bundle_idx]))
                max_mtr[bundle_idx] = np.max((np.nanmax(result['MTR']), max_mtr[bundle_idx]))
                min_mtsat[bundle_idx] = np.min((np.nanmin(result['MTsat']), min_mtsat[bundle_idx]))
                max_mtsat[bundle_idx] = np.max((np.nanmax(result['MTsat']), max_mtsat[bundle_idx]))
                min_ihmtr[bundle_idx] = np.min((np.nanmin(result['ihMTR']), min_ihmtr[bundle_idx]))
                max_ihmtr[bundle_idx] = np.max((np.nanmax(result['ihMTR']), max_ihmtr[bundle_idx]))
                min_ihmtsat[bundle_idx] = np.min((np.nanmin(result['ihMTsat']), min_ihmtsat[bundle_idx]))
                max_ihmtsat[bundle_idx] = np.max((np.nanmax(result['ihMTsat']), max_ihmtsat[bundle_idx]))

                ax[row, col].set_ylim(0.975 * min_mtr[bundle_idx],
                                    1.025 * max_mtr[bundle_idx])
                ax[row, col].set_yticks([np.round(min_mtr[bundle_idx], decimals=1),
                                        np.round(max_mtr[bundle_idx], decimals=1)])
                ax[row, col].set_xlim(0, 90)
                # ax[row, col].tick_params(axis='y', labelcolor="C0")
                ax[row, col + 1].set_ylim(0.975 * min_mtsat[bundle_idx],
                                        1.025 * max_mtsat[bundle_idx])
                ax[row, col + 1].set_yticks([np.round(min_mtsat[bundle_idx], decimals=1),
                                            np.round(max_mtsat[bundle_idx], decimals=1)])
                ax[row, col + 1].set_xlim(0, 90)
                # ax[row, col + 1].tick_params(axis='y', labelcolor="C2")
                ax[row, col + 2].set_ylim(0.975 * min_ihmtr[bundle_idx],
                                        1.025 * max_ihmtr[bundle_idx])
                ax[row, col + 2].set_yticks([np.round(min_ihmtr[bundle_idx], decimals=1),
                                            np.round(max_ihmtr[bundle_idx], decimals=1)])
                ax[row, col + 2].set_xlim(0, 90)
                ax[row, col + 3].set_ylim(0.975 * min_ihmtsat[bundle_idx],
                                        1.025 * max_ihmtsat[bundle_idx])
                ax[row, col + 3].set_yticks([np.round(min_ihmtsat[bundle_idx], decimals=1),
                                            np.round(max_ihmtsat[bundle_idx], decimals=1)])
                ...
                min_mtr[-1] = np.min((np.nanmin(result['MTR']), min_mtr[-1]))
                max_mtr[-1] = np.max((np.nanmax(result['MTR']), max_mtr[-1]))
                min_mtsat[-1] = np.min((np.nanmin(result['MTsat']), min_mtsat[-1]))
                max_mtsat[-1] = np.max((np.nanmax(result['MTsat']), max_mtsat[-1]))
                min_ihmtr[-1] = np.min((np.nanmin(result['ihMTR']), min_ihmtr[-1]))
                max_ihmtr[-1] = np.max((np.nanmax(result['ihMTR']), max_ihmtr[-1]))
                min_ihmtsat[-1] = np.min((np.nanmin(result['ihMTsat']), min_ihmtsat[-1]))
                max_ihmtsat[-1] = np.max((np.nanmax(result['ihMTsat']), max_ihmtsat[-1]))

                ax[row, col].set_ylim(0.975 * min_mtr[-1],
                                    1.025 * max_mtr[-1])
                ax[row, col].set_yticks([np.round(min_mtr[-1], decimals=1),
                                        np.round(max_mtr[-1], decimals=1)])
                ax[row, col].set_xlim(0, 90)
                # ax[row, col].tick_params(axis='y', labelcolor="C0")
                ax[row, col + 1].set_ylim(0.975 * min_mtsat[-1],
                                        1.025 * max_mtsat[-1])
                ax[row, col + 1].set_yticks([np.round(min_mtsat[-1], decimals=1),
                                            np.round(max_mtsat[-1], decimals=1)])
                ax[row, col + 1].set_xlim(0, 90)
                # ax[row, col + 1].tick_params(axis='y', labelcolor="C2")
                ax[row, col + 2].set_ylim(0.975 * min_ihmtr[-1],
                                        1.025 * max_ihmtr[-1])
                ax[row, col + 2].set_yticks([np.round(min_ihmtr[-1], decimals=1),
                                            np.round(max_ihmtr[-1], decimals=1)])
                ax[row, col + 2].set_xlim(0, 90)
                ax[row, col + 3].set_ylim(0.975 * min_ihmtsat[-1],
                                        1.025 * max_ihmtsat[-1])
                ax[row, col + 3].set_yticks([np.round(min_ihmtsat[-1], decimals=1),
                                            np.round(max_ihmtsat[-1], decimals=1)])
                ax[row, col + 3].set_xlim(0, 90)
