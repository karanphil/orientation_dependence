...
def main():
    ...
    for i in range(maps.shape[-1]):
        ...
        colorbar = ax[i, 0].imshow(y_mask, cmap=cm.navia, vmin=0, vmax=vmax, interpolation='none')
        ax[i, 1].imshow(x_mask, cmap=cm.navia, vmin=0, vmax=vmax, interpolation='none')
        ax[i, 2].imshow(z_mask, cmap=cm.navia, vmin=0, vmax=vmax, interpolation='none')
